...
def ficha_societaria(key, data):
    ...
    st.divider()
    st.markdown("### 🎓 Profundización académica")

    if data.get("debate_doctrinario"):
        with st.expander("🗣️ Debate doctrinario"):
            st.markdown(data["debate_doctrinario"])

    if data.get("jurisprudencia_clave"):
        with st.expander("⚖️ Jurisprudencia clave"):
            for caso in data["jurisprudencia_clave"]:
    ...
    if data.get("conflictos_frecuentes"):
        with st.expander("⚔️ Conflictos societarios frecuentes"):
            for c in data["conflictos_frecuentes"]:
                st.markdown(f"- {c}")

    if data.get("inoponibilidad_velo"):
        with st.expander("🎭 Inoponibilidad y velo societario (art. 54, 3er párr. LGS)"):
            st.markdown(data["inoponibilidad_velo"])
...
# --------------------------------------------------------------------------
    ...
    if query:
        ...
        for key in ORDEN_TIPOS:
            d = SOCIEDADES[key]
            jurisprudencia_txt = " ".join(
                f"{c.get('caso','')} {c.get('tribunal','')} {c.get('sintesis','')}"
                for c in d.get("jurisprudencia_clave", [])
            )
            bloque_texto = " ".join(
                [
                    ...
                    " ".join(d["normativa"]),
                    d.get("debate_doctrinario", ""),
                    jurisprudencia_txt,
                    " ".join(d.get("conflictos_frecuentes", [])),
                    d.get("inoponibilidad_velo", ""),
                ]
            ).lower()
